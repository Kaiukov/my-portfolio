...
| `portfolio performance` | `portfolio-ts performance` | **implemented** | Calls `portfolio_performance_sql(as_of_date, benchmark, from_date)` — PostgreSQL owns all TWR/Sharpe/MDD/benchmark calculations. Public output names the TWR-equivalent gain `twr_gain_equivalent_usd`, cash-flow-inclusive growth `portfolio_growth_pct`, and inflation-adjusted growth `real_portfolio_growth_pct`; SQL return columns remain unchanged. Also includes `median_monthly_return`, `period_returns` (1M/3M/6M/YTD/1Y/SII via `portfolio_period_returns_sql`) and `rolling_12m_returns` (via `portfolio_rolling_returns_sql`). All period/rolling returns are TWR (geometric-linked investment_return). Supports `--as-of-date`, `--benchmark`, `--from-date`, `--period` (ytd/1y/6m/3m). |
