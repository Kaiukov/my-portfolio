...
Phase 1 of issue #56 — validate that SQL-side discovery and checkpoint
functions produce correct results against known transaction fixtures.
"""
import sys
...
def _seed_prices(db, tickers, start="2026-01-01", end="2026-12-31"):
    """Pre-populate price cache for test tickers to allow BUY/SELL transactions."""
    import pandas as pd
...
# ---------------------------------------------------------------------------
# discover_assets_and_currencies (via SQL: discover_required_tickers_sql)
# ---------------------------------------------------------------------------
...
def test_discover_usd_only_portfolio(service):
    ...
    result = service.discover_assets_and_currencies()
...
# ---------------------------------------------------------------------------
# get_required_price_checkpoints (via SQL: get_required_price_checkpoints_sql)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# New import paths created by Phase 3 package restructure
# ---------------------------------------------------------------------------

def test_core_domain_import():
    from portfolio_db.core.domain import get_asset_type, BASE_CURRENCY
...
def test_core_errors_import():
    from portfolio_db.core.errors import PriceDataUnavailableError
    assert issubclass(PriceDataUnavailableError, ValueError)


def test_adapters_cli_response_import():
    from portfolio_db.adapters.cli.response import success, error
    assert callable(success)
    assert callable(error)


def test_postgres_repositories_import():
    from portfolio_db.postgres.repositories import PortfolioDatabase, resolve_db_target
    assert callable(resolve_db_target)
    assert PortfolioDatabase is not None


def test_services_import():
    from portfolio_db.services import PortfolioService as PS
    from portfolio_db.services.price_service import PriceService as PrS
