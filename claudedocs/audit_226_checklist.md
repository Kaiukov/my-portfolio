...
2. `avg_drawdown_duration` previously computed the episode grouping after filtering to positive drawdown rows, so separate drawdown episodes collapsed into one group and returned the sum of durations. Before: fixture SQL returned `362`. After: grouping is computed over the full drawdown series and positive rows are filtered afterward, returning the mean episode duration `181`.
3. `spy_twr_pct` previously used `((spy_e - spy_s) / spy_s) * 100.0`, but `spy_s` came from the first benchmark row with `prev_price IS NOT NULL`, so the fixture started SPY at `102` instead of the true window start `100`. Before: fixture SQL returned `27.4509803922`. After: `spy_twr_pct` uses the already-correct geometric `d.spy_twr * 100.0`, returning `30.0` and matching `spy_cagr_pct`'s source.
