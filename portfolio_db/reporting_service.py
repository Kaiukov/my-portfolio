...
class ReportingService:
    ...
    def _build_position_snapshot(self, as_of_date, price_context: dict, cash_snapshot: list, include_closed, trade_actions, price_data_unavailable_error_cls) -> list:
        ...
        for symbol, pos_data in positions.items():
            ...
            # Convert non-USD stock prices to USD
            if last_price is not None and asset_type in ASSET_TYPE_TO_CASH:
                last_price *= fx_rate(ASSET_TYPE_TO_CASH[asset_type])
            ...
            if shares > 0 and last_price and price_series is not None:
                try:
                    ...
                    if len(price_history) > 1:
                        ...
                        # Apply per-day FX conversion for non-USD stocks
                        if asset_type in ASSET_TYPE_TO_CASH:
                            fx_symbol = ASSET_TYPE_TO_CASH[asset_type]
