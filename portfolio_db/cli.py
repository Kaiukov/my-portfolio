...
DEFAULT_DB_HELP = "DuckDB file (default: portfolio.db)"
...
TOP_LEVEL_HELP = """Portfolio tracking with DuckDB.

All commands emit JSON only.
Per-command --db defaults to portfolio.db unless you override it.
...
Write / maintenance commands:
  ...
  backup          Copy the DuckDB file to a backup.
  init            Create or initialize the DuckDB schema.
...
Examples:
  ...
  portfolio migrate --csv ./exports/q1-transactions.csv --db ./portfolio.db
""")
@click.option("--csv", default="yfiance-transactions/transactions.csv", show_default=True, help=MIGRATE_CSV_HELP)
@click.option("--db", default="portfolio.db", show_default=True, help=DEFAULT_DB_HELP)
def migrate(csv, db):
